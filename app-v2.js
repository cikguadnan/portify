...
$("#saveExp").onclick=()=>{const title=$("#expTitle").value.trim();if(!title)return alert("Add a title first.");const entry={title,type:$("#expType").value,year:$("#expYear").value.trim(),description:$("#expDescription").value.trim(),text:$("#expText").value.trim(),skills:$("#expSkills").value.trim(),featured:$("#expFeatured").checked};const index=+$("#expEditIndex").value;if(index>=0)data.experiences[index]=entry;else data.experiences.unshift(entry);$("#expModal").classList.remove("show");renderExp();renderStats();queueSave()};
